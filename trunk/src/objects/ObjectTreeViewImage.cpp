		...
		if (type == Event::AttachedToObject)
		{
			...
			if (treeViewNode != NULL)
			{
				...
				else
				{
					this->_treeView = NULL;
					this->_treeViewNode = NULL;
					hlog::errorf(aprilui::logTag, "TreeViewImage '%s' cannot be assigned to TreeViewNode '%s', it already exist!", this->name.cStr(), this->parent->getName().cStr());
				}
			}
			else if (this->parent != NULL)
			{
				this->_treeView = NULL;
				this->_treeViewNode = NULL;
				hlog::errorf(aprilui::logTag, "TreeViewImage '%s' not attached to object of class TreeViewNode!", this->name.cStr());
			}
		}
	}
