...
namespace aprilui
{
	namespace Animators
	{
		...
		TileImage* TileScrollerX::_getParentsTileImage()
		{
			...
			if (imageBox == NULL)
			{
				hlog::errorf(aprilui::logTag, "Animators::TileScrollerX: parent object '%s' not a subclass of Objects::ImageBox!", parentName.c_str());
				return NULL;
			}
			...
			if (image == NULL)
			{
				hlog::errorf(aprilui::logTag, "Animators::TileScrollerX: Image '%s' in ImageBox '%s' not a subclass of TileImage!", imageName.c_str(), parentName.c_str());
				return NULL;
			}
			return image;
		}

		void TileScrollerX::update(float timeDelta)
		{
