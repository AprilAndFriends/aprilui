...
namespace aprilui
{
	namespace Animators
	{
		...
		TileImage* TileScrollerY::_getParentsTileImage()
		{
			...
			if (imageBox == NULL)
			{
				hlog::errorf(aprilui::logTag, "Animators::TileScrollerY: parent object '%s' not a subclass of Objects::ImageBox!", parentName.c_str());
				return NULL;
			}
			...
			if (image == NULL)
			{
				hlog::errorf(aprilui::logTag, "Animators::TileScrollerY: Image '%s' in ImageBox '%s' not a subclass of TileImage!", imageName.c_str(), parentName.c_str());
				return NULL;
			}
			return image;
		}

		void TileScrollerY::update(float timeDelta)
		{
